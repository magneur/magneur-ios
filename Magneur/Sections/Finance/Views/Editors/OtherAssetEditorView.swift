...
/// Editor for other asset types
struct OtherAssetEditorView: View {
    ...
    var body: some View {
        ItemEditorWrapper(
            title: item.id == nil ? "Add Other Asset" : "Edit Other Asset",
            canSave: canSave,
            onSave: saveItem
        ) {
            EditorComponents.InputSection(title: "Asset Name") {
                TextField("e.g., Art Collection", text: $item.name)
            ...
            EditorComponents.InputSection(title: "Notes (Optional)") {
                TextField("Describe the asset...", text: $item.notes, axis: .vertical)
                    .lineLimit(3...6)
                    .foregroundStyle(.white)
            }
        }
...
#Preview {
    OtherAssetEditorView(item: .otherAsset()) { }
}
