...
struct StockEditorView: View {
    ...
    var body: some View {
        ItemEditorWrapper(
            title: item.id == nil ? "Add Stock" : "Edit Stock",
            canSave: canSave,
            onSave: saveItem
        ) {
            EditorComponents.InputSection(title: "Ticker Symbol") {
                TextField("e.g., AAPL", text: Binding(
                    get: { item.ticker ?? "" },
                    set: {
            ...
            Text("Stock prices will be fetched automatically from Yahoo Finance.")
                .font(.caption)
