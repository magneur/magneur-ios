...
/// Editor for business loan liabilities
struct BusinessLoanEditorView: View {
    ...
    var body: some View {
        ItemEditorWrapper(
            title: item.id == nil ? "Add Business Loan" : "Edit Business Loan",
            canSave: canSave,
            onSave: saveItem
        ) {
            EditorComponents.InputSection(title: "Loan Name") {
                TextField("e.g., SBA Loan", text: $item.name)
...
#Preview {
    BusinessLoanEditorView(item: .businessLoan()) { }
}
