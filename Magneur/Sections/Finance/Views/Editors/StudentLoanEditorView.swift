...
/// Editor for student loan liabilities
struct StudentLoanEditorView: View {
    ...
    var body: some View {
        ItemEditorWrapper(
            title: item.id == nil ? "Add Student Loan" : "Edit Student Loan",
            canSave: canSave,
            onSave: saveItem
        ) {
            EditorComponents.InputSection(title: "Loan Name") {
                TextField("e.g., Federal Student Loan", text: $item.name)
...
#Preview {
    StudentLoanEditorView(item: .studentLoan()) { }
}
