...
struct VehicleEditorView: View {
    ...
    var body: some View {
        ItemEditorWrapper(
            title: item.id == nil ? "Add Vehicle" : "Edit Vehicle",
            canSave: canSave,
            onSave: saveItem
        ) {
            EditorComponents.InputSection(title: "Vehicle Name") {
                TextField("e.g., 2022 Toyota Camry", text: $item.name)
...
#Preview {
    VehicleEditorView(item: .vehicle()) { }
}
