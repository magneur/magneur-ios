...
/// Editor for mortgage liabilities
struct MortgageEditorView: View {
    ...
    var body: some View {
        ItemEditorWrapper(
            title: item.id == nil ? "Add Mortgage" : "Edit Mortgage",
            canSave: canSave,
            onSave: saveItem
        ) {
            EditorComponents.InputSection(title: "Mortgage Name") {
                TextField("e.g., Primary Home Mortgage", text: $item.name)
                    .foregroundStyle(.white)
            }

            EditorComponents.InputSection(title: "Property Address (Optional)") {
...
#Preview {
    MortgageEditorView(item: .mortgage()) { }
}
