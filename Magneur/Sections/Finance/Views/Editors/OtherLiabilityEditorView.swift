//
//  OtherLiabilityEditorView.swift
//  Magneur
//
...
/// Editor for other liability types
struct OtherLiabilityEditorView: View {
    ...
    var body: some View {
        ItemEditorWrapper(
            title: item.id == nil ? "Add Other Liability" : "Edit Other Liability",
            canSave: canSave,
            onSave: saveItem
        ) {
            EditorComponents.InputSection(title: "Liability Name") {
                TextField("e.g., Family Loan", text: $item.name)
            ...
            EditorComponents.InputSection(title: "Original Amount (Optional)") {
            ...
            EditorComponents.InputSection(title: "Notes (Optional)") {
                TextField("Describe the liability...", text: $item.notes, axis: .vertical)
                    .lineLimit(3...6)
                    .foregroundStyle(.white)
            }
        }
...
#Preview {
    OtherLiabilityEditorView(item: .otherLiability()) { }
}
