...
/// Editor for medical debt liabilities
struct MedicalDebtEditorView: View {
    ...
    var body: some View {
        ItemEditorWrapper(
            title: item.id == nil ? "Add Medical Debt" : "Edit Medical Debt",
            canSave: canSave,
            onSave: saveItem
        ) {
            EditorComponents.InputSection(title: "Description") {
                TextField("e.g., Hospital Bill", text: $item.name)
...
#Preview {
    MedicalDebtEditorView(item: .medicalDebt()) { }
}
